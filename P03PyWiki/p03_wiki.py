...
def _do_it_twice(func):
    ...
    def twice_func(*args, **kwargs):
        try:
            ...
            if "too many" in str(ex).lower():
                print("Taking a 5 second break")
                time.sleep(5)
                try:
                    return func(*args, **kwargs)
                except Exception as ex2:
                    if "too many" in str(ex2).lower():
                        print("Taking a 20 second break")
                        time.sleep(20)
                        try:
                            return func(*args, **kwargs)
                        except Exception as ex3:
                            if "too many" in str(ex3).lower():
...
if "INSCRYPTION_PATH" not in os.environ:
    raise RuntimeError(
        "Environment variable `INSCRYPTION_PATH` not set to the location of Inscryption"
    )

if not os.path.exists(os.path.join(os.environ["INSCRYPTION_PATH"], "cardexports")):
    raise RuntimeError(
        "No metadata found in Inscryption game directory. Run P03 in Kaycee's Mod, go to the main map screen in-game, and enter the CTRL+SHIFT+E key combination. Wait for it to finish, close the game, then come back here."
    )
...
def login() -> mwclient.Site:
    site = mwclient.Site(
        "p03kcmod.wiki.gg",
        httpauth=(os.environ["PYWIKI_DEV_USER"], os.environ["PIWIKI_DEV_PASSWORD"]),
        path="/",
    )
...
def _special_enums():
    with open(
        os.path.join(
            os.environ["INSCRYPTION_PATH"], "cardexports", "special_enums.json"
        )
    ) as f:
        return json.load(f)
...
def get_ability_dictionary():
    ...
    for fname in glob.glob(
        os.path.join(os.environ["INSCRYPTION_PATH"], "cardexports", "ability*.json")
    ):
...
def get_hacky_single_fuel_cost_info(line: str):
    name = ""
    fuel_cost = 0
    for l in line.splitlines():
        if "CardManager.New" in l:
            name = "P03KCMXP3_" + l.split('"')[1]
        if "SetStartingFuel" in l:
            fuel_cost = int(l.split("(")[1][0])
    return name, fuel_cost


@functools.cache
def get_hacky_fuel_cost_info():
    all_fuels = {}
    with open(os.path.join("..", "P03ExpansionPack3", "Cards.cs")) as f:
        data = f.read()
    dlines = data.split(";")
    dlines = [l for l in dlines if "CardManager.New" in l]
    for l in dlines:
        n, fu = get_hacky_single_fuel_cost_info(l)
        all_fuels[n] = fu
    return all_fuels
...
def get_card_data(card_name):
    with open(
        os.path.join(
            os.environ["INSCRYPTION_PATH"], "cardexports", f"card_{card_name}.json"
        )
    ) as f:
        data = json.load(f)

    fci = get_hacky_fuel_cost_info()
    data["startingFuel"] = fci.get(card_name, 0)
    return data
...
def get_extra_card_properties(cardname, index):
    ...
    expansion_pack = (
        "Expansion Pack 3"
        if cardname.startswith("P03KCMXP3")
        else (
            "Expansion Pack 2"
            if cardname.startswith("P03KCMXP2")
            else (
                "Expansion Pack 1"
                if cardname.startswith("P03KCMXP1")
                else (
                    "P03 in Kaycee's Mod"
                    if cardname.startswith("P03KCM")
                    else "Base Game"
                )
            )
        )
    ...
    if data["cost"] > 0:
        cost = f"{data['cost']} Blood"
    elif data["bonesCost"] > 0:
        cost = f"{data['bonesCost']} Bones"
    elif len(data["gemsCost"]) > 0:
        mapping = {0: "Green", 1: "Orange", 2: "Blue"}
        cost = ", ".join([mapping[i] for i in data["gemsCost"]]) + " Gem"
    else:
        cost = f"{data['energyCost']} Energy"

    abilityString = (
        "{{Pagelist|"
        + "|".join(ability_dict[ab]["rulebookName"] for ab in data["abilities"])
        + "}}"
    )
...
def generate_ability_page(ability, raise_errors=False):
    ...
    try:
        ...
        desc = data["rulebookDescription"].replace(
            "[creature]", "a card bearing this sigil"
        )
        ...
        cards = [
            c["displayedName"]
            for c in get_card_dictionary().values()
            if "!" not in c["displayedName"]
            and _special_enums()["CardTemple"][str(c["temple"])] == "Tech"
            and "abilities" in c
            and data["ability"] in c["abilities"]
        ]
    ...
    try:
        ...
        if "evolveParams" in data:
            if card_has_ability(data, "fledgling"):
                ...
                all_card_datas |= {
                    "Evolution": get_extra_card_properties(
                        data["evolveParams"]["evolution"], len(all_card_datas) + 1
                    )
                }
            elif (
                card_has_ability(data, "transformer")
                or card_has_ability(data, "transforms when powered")
                or card_has_ability(data, "transforms when unpowered")
            ):
                is_root = is_obtainable
                all_card_datas |= {
                    "Transform": get_extra_card_properties(
                        data["evolveParams"]["evolution"], len(all_card_datas) + 1
                    )
                }
        ...
        if "iceCubeParams" in data:
            if card_has_ability(data, "frozen away"):
                is_root = is_obtainable
                all_card_datas |= {
                    "Dies Into": get_extra_card_properties(
                        data["iceCubeParams"]["creatureWithin"], len(all_card_datas) + 1
                    )
                }
...
def edit_page_contents(
    ...
    try:
        page: Page = get_page_safe(site, page_name)
    except Exception as ex:
        print(f"Failed to get page {page_name}: {ex}")
        return
...
def publish_card_pages(overwrite_if_exists=False, filter=None):
    generated_pages = {}
    client = login()
    for fname in glob.glob(
        os.path.join(os.environ["INSCRYPTION_PATH"], "cardexports", "card*.json")
        ...
        if filter is not None and filter not in cname:
            continue
...
def publish_ability_pages(overwrite_if_exists=False):
    ...
    for fname in glob.glob(
        ...
        if page_contents != None and number_of_cards > 0:
            edit_page_contents(
                client, aname, page_contents, overwrite_if_exists=overwrite_if_exists
            )
...
def unobtainable_cards():
    ...
    unob_cards = list(
        p.name
        for p in client.categories["Unobtainable Cards"]
        if p.name not in transformer_cards
    )
    return unob_cards
    ...
    if "--skip-touch" not in sys.argv:
        ...
        for cat in client.categories:
            if "Card" in cat.name or "Art" in cat.name:
                update_card_categories(
                    cat.name.replace("Category:", ""), "Art" in cat.name
                )
...
if __name__ == "__main__":
    ...
    if "--gen-card-pages" in sys.argv:
        publish_card_pages()
    if "--gen-ability-pages" in sys.argv:
        publish_ability_pages()
    if "--regen-xp3" in sys.argv:
        publish_card_pages(True, "P03KCMXP3")
