...
namespace Infiniscryption.P03KayceeRun.Faces
{
    ...
    public class P03LivesFace : ManagedBehaviour
    {
        ...
        public IEnumerator ShowLives(int currentLives, int newLives)
        {
            ...
            float sign = Mathf.Sign(newLives - currentLives);
            while (currentLives != newLives)
            {
                yield return new WaitForSeconds(0.6f);
                UpdateText(currentLives);
                currentLives += (int)(1f * sign);
                AudioController.Instance.PlaySound3D("robo_scale_tick", MixerGroup.TableObjectsSFX, P03AnimationController.Instance.HeadParent.position, 1f, 0f, new AudioParams.Pitch(1f + ((newLives - currentLives) * -0.01f)), null, null, null, false);
            }
        ...
        public static void CreateLivesFace(ref P03AnimationController __instance)
        {
            ...
            P03LivesFaceObject = Instantiate(currencyFace, currencyFace.transform.parent);
